...
public class ImprovedParser {
     ...
     * 由于当 lexer.match(ch04.Lexer.PLUS) 判断成立时，expr_prime() 有递归调用，因此其等价于
     * while (lexer.match(ch04.Lexer.PLUS)) {
     *   lexer.advance();
     *   term();
     * }
     * 
    private void expr_prime() {
    	...
    	if (lexer.match(ch04.Lexer.PLUS)) {
    		lexer.advance();
    		term();
    		expr_prime();
    	}
    	else if (lexer.match(ch04.Lexer.UNKNOWN_SYMBOL)) {
    		isLegalStatement = false;
    		System.out.println("unknow symbol: " + lexer.yytext);
    		return;
    	}
    ...
    */
    ...
    private void term_prime() {
    	...
    	if (lexer.match(ch04.Lexer.TIMES)) {
    		lexer.advance();
    		factor();
    		term_prime();
    	}
