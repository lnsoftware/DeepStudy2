...
public class MBeanRegistrar {
    ...
    public static ClientConnectionManager findClientConnMgrByMbeanName(final String mbeanName) {
        if (CONN_MGR_MAP.isEmpty()) {
            return null;
        }
        if (mbeanName == null) {
            return CONN_MGR_MAP.entrySet().iterator().next().getKey();
        }
    ...
    private static String createMbeanName(final String mbeanName) {
        if (mbeanName == null || mbeanName.isEmpty()) {
            return Thread.currentThread().getId() + "_" + new SimpleDateFormat("yyyyMMdd_HHmmss_SSS").format(new Date());
        }
        return mbeanName;
    }
