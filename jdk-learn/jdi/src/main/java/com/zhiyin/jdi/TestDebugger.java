...
import com.zhiyin.jdi.DebugeeConfig;
...
public class TestDebugger {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Connector.Argument portArg = (Connector.Argument) arguments.get(PORT);
         ...
         hostArg.setValue(DebugeeConfig.BreakpointConfig.ServerHost);
        portArg.setValue(DebugeeConfig.BreakpointConfig.ServerPort);

        Connector.Argument timeoutArg = (Connector.Argument) arguments.get("timeout");

        timeoutArg.setValue("4000");
        ...
        // 三、取得要关注的类和方法
        List<ReferenceType> classesByName = vm.classesByName(DebugeeConfig.BreakpointConfig.BreakpointClassName);
        if (classesByName == null || classesByName.size() == 0) {
            System.out.println("No class found" + DebugeeConfig.BreakpointConfig.BreakpointClassName);
            return;
        }
...
//        Method method = methodsByName.get(0);
        ...
        // 设置断点信息，断点行数
        List<Location> locations = classesByName.get(0).locationsOfLine(DebugeeConfig.BreakpointConfig.BreakpointLineNum);
...
//            System.out.println("Reach Method printHello of test.com.zhiyin.jdi.DemoServer.Test");
            ...
            //变量是int类型
            LocalVariable localVariable1 = stackFrame.visibleVariableByName(DebugeeConfig.BreakpointConfig.LocalVariableName);
            Value value1 = stackFrame.getValue(localVariable1);
            int i = ((IntegerValue) value1).intValue();

            System.out.println("The local variable " + DebugeeConfig.BreakpointConfig.LocalVariableName + " is " + i);
            eventSet.resume();
